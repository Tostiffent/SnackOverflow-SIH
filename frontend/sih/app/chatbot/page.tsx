...
const ChatBot = () => {
  ...
  useEffect(() => {
    const socket = io(
      "https://orange-space-memory-4pvjvr59w7rfqg6q-5000.app.github.dev/"
    );
    socket.on("connect", () => {
    ...
    socket.on("response", (data: any) => {
      ...
      setMessages((oldArray: any) => [...oldArray, { msg: data.res, id: "2" }]);
    });
  ...
  async function sendMsg() {
    if (ws) {
      ws.emit("send_message", { msg: message, id: "1" });
      setMessages((oldArray: any) => [...oldArray, { msg: message, id: "1" }]);
      setMessage("");
    }
  }
