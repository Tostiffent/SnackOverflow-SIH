...
import { Send, Calendar, Tag, CreditCard, Sparkles, Sun, Moon, MicOff, Mic } from 'lucide-react'
...
function ChatbotPage() {
  const [messages, setMessages] = useState([
    { sender: 'bot', content: 'Hello welcome to the museum, Say hello 👋 to get started ' },
  ])
  ...
  const connectWebSocket = () => {
    ...
    socket.on("response", (data) => {
      ...
      newInfo.name = newInfo.name !== "" && oldInfo.name !== newInfo.name ? newInfo.name : oldInfo.name
      newInfo.show = newInfo.show !== "" && oldInfo.show !== newInfo.show ? newInfo.show : oldInfo.show
      newInfo.number_of_tickets = newInfo.number_of_tickets !== 0 && oldInfo.number_of_tickets !== newInfo.number_of_tickets ? newInfo.number_of_tickets : oldInfo.number_of_tickets
      newInfo.total_amount = newInfo.total_amount !== 0 && oldInfo.total_amount !== newInfo.total_amount ? newInfo.total_amount : oldInfo.total_amount
      setBookingInfo(newInfo)
      setMessages((oldArray) => [...oldArray, { sender: 'bot', content: data.res.msg, toolCall: data.res.toolCall }])
  ...
  const sendMsg = (ms: string) => {
    if (ws && ws.connected) {
      ...
      setMessages((oldArray) => [...oldArray, { sender: 'user', content: ms, toolCall: { type: "none" } }])
      setInput('')
    } else {
      console.error("WebSocket is not connected")
      setMessages((oldArray) => [...oldArray, { sender: 'bot', content: "Sorry, I'm having trouble connecting. Please try again in a moment." }])
    }
  }
  ...
  return (
    <div className={`flex h-screen w-full ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} transition-colors duration-500`}>
      ...
      <div className="flex flex-col w-full max-w-screen-2xl mx-auto p-4 lg:p-6 h-full">
        <div className={`flex items-center justify-between p-4 bg-gradient-to-r from-purple-600 to-pink-600 rounded-t-xl`}>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              aria-label={isVoiceBotActive ? "Deactivate voice bot" : "Activate voice bot"}
            >
        ...
        <div className={`flex flex-1 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-b-xl overflow-hidden transition-colors duration-500`}>
        <div className="flex flex-col w-2/3 border-r border-gray-200 dark:bg-gray-900">
            {isVoiceBotActive ? (
              <div className="flex-1 overflow-auto p-4">
                <PipecatWebSocketClient setCall={() => setIsVoiceBotActive(false)} isDarkMode={isDarkMode} />
              </div>
            ) : (
              <>
                <div className="flex-1 overflow-auto p-4 space-y-4">
                  {messages.map((message, index) => (
                    ...
                    >
                      ...
                      >
                        {message.toolCall && message.toolCall.type === "events" ? (
                      <div>
                        <p>Select the event to book:</p>
                        <div className="mt-2">
                          {message.toolCall.events.map((event: string, eventIndex: number) => (
                            <div
                              key={eventIndex}
                    ...
                    ) : message.toolCall && message.toolCall.type === "tickets" ? (
                      <div className="flex flex-col items-start space-y-2">
                        <span className="text-white font-semibold">
                          Select number of tickets required (max: {maxTickets})
                        </span>
                ...
                <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex items-center bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {const g=
                          handleSend()
                        }
                      }}
                      placeholder={isConnecting ? "Connecting..." : "Type a message..."}
                      className={`flex-grow p-3 bg-transparent focus:outline-none ${isDarkMode ? 'text-black placeholder-gray-400' : 'text-gray-900 placeholder-gray-500'}`}
                      disabled={isConnecting}
                    />
                    <button 
                      onClick={handleSend} 
                      className={`p-3 ${isConnecting ? 'text-gray-400 cursor-not-allowed' : 'text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-300'}`}
                      disabled={isConnecting}
                    >
                      <Send size={20} />
                    </button>
...
<span className="font-semibold">
              ...
              <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                <h4 className="font-semibold mb-2 flex items-center">
                  <Tag size={16} className="mr-2 text-green-500" /> Have a promo code?
                </h4>
                <div className="flex">
                  <input
                    type="text"
                    placeholder="Promo code"
                    className={`flex-grow p-2 rounded-l-md ${isDarkMode ? 'bg-gray-600 text-white' : 'bg-white text-gray-900'} focus:outline-none`}
                  />
              ...
              <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                <h4 className="font-semibold mb-2 flex items-center">
                  <CreditCard size={16} className="mr-2 text-purple-500" /> Summary
                </h4>
                <div className="space-y-2">
                  <div className="flex justify-between">
                  <span>General ticket (x{bookingInfo.number_of_tickets})</span>
                  ...
                  <div className="flex justify-between font-bold pt-2 border-t border-gray-300 dark:border-gray-600">
                    ...
                    <span className="text-green-600 dark:text-green-400">₹ {bookingInfo.total_amount}</span>
                  </div>
                </div>
              </div>
            </div>
